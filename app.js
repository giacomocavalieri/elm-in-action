...
var $author$project$PhotoGroove$init = function (flags) {
	...
	var initialCommand = $elm$http$Http$get(
		{
			expect: A2(
...
var $author$project$PhotoGroove$Error = function (a) {
	return {$: 'Error', a: a};
};
...
var $author$project$PhotoGroove$SelectedPhoto = function (a) {
	return {$: 'SelectedPhoto', a: a};
};
var $author$project$PhotoGroove$maxValue = 11;
...
var $author$project$PhotoGroove$applyFilters = function (model) {
	...
	switch (_v0.$) {
		case 'Loaded':
			var photo = _v0.b;
			var url = $author$project$PhotoGroove$urlPrefix + ('large/' + photo.url);
			var filters = _List_fromArray(
				[
					{amount: model.hue / $author$project$PhotoGroove$maxValue, name: 'Hue'},
					{amount: model.ripple / $author$project$PhotoGroove$maxValue, name: 'Ripple'},
					{amount: model.noise / $author$project$PhotoGroove$maxValue, name: 'Noise'}
				]);
...
var $author$project$PhotoGroove$selectPhoto = F2(
	function (photo, status) {
		switch (status.$) {
			case 'Loaded':
				var photos = status.a;
				return A2($author$project$PhotoGroove$Loaded, photos, photo);
			case 'Loading':
				return status;
			default:
				return status;
			...
			case 'SelectedPhoto':
				...
				return $author$project$PhotoGroove$applyFilters(
					_Utils_update(
						model,
						{
							status: A2($author$project$PhotoGroove$selectPhoto, photo, model.status)
						}));
				...
				switch (_v1.$) {
					case 'Loaded':
						if (_v1.a.b) {
							...
							var generator = A2($elm$random$Random$uniform, firstPhoto, otherPhotos);
							var cmd = A2($elm$random$Random$generate, $author$project$PhotoGroove$SelectedPhoto, generator);
							return _Utils_Tuple2(model, cmd);
						} else {
							return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
						}
				...
				if (message.a.$ === 'Ok') {
					if (message.a.a.b) {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									status: $author$project$PhotoGroove$Error('0 photos found')
								}),
							$elm$core$Platform$Cmd$none);
					}
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								status: $author$project$PhotoGroove$Error('Server error!')
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'GotActivity':
				var activity = message.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{activity: activity}),
...
var $author$project$PhotoGroove$sizeToString = function (thumbnailSize) {
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$core$String$fromInt($author$project$PhotoGroove$maxValue)),
							A2(
...
var $author$project$PhotoGroove$viewSizeChooser = F2(
	function (selectedSize, thumbnailSize) {
		return A2(
			$elm$html$Html$label,
			_List_Nil,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							...
							$author$project$PhotoGroove$ClickedSize(thumbnailSize)),
							$elm$html$Html$Attributes$checked(
							_Utils_eq(thumbnailSize, selectedSize))
						]),
					_List_Nil),
					$elm$html$Html$text(
...
var $author$project$PhotoGroove$viewThumbnail = F2(
	function (selectedPhoto, thumbnail) {
		return A2(
			$elm$html$Html$img,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							_Utils_Tuple2(
							'selected',
							_Utils_eq(selectedPhoto, thumbnail))
						])),
					$elm$html$Html$Events$onClick(
					$author$project$PhotoGroove$SelectedPhoto(thumbnail))
				]),
			_List_Nil);
	});
var $author$project$PhotoGroove$viewLoaded = F3(
	function (photos, selectedPhoto, model) {
		return _List_fromArray(
			[
				A2(
				$elm$html$Html$h1,
		...
		function () {
			...
			switch (_v0.$) {
				...
				case 'Loaded':
					...
					return A3($author$project$PhotoGroove$viewLoaded, photos, selectedPhoto, model);
				default:
